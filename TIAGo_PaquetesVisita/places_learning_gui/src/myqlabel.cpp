...
void MyQLabel :: paintEvent(QPaintEvent * e)
{
	...
	float x_converted = ((*current_x) - map_origin_x)/(map_width_relation*map_resolution);
        float y_converted = (map_origin_y +0.3 + input_map_height*map_resolution - (*current_y))/(map_height_relation*map_resolution);
	...
	// draw rooms and places
        for(unsigned int i=0;i<rooms->rooms_vector.size();i++)
        {
               ...
               {
                  Point2D corner = rooms->rooms_vector[i].vertices_vector[j];

                   QPoint vertex;
		   //float vertex_x_converted = (corner.x - map_origin_x)/(map_width_relation*map_resolution); (original)
		   
		   float vertex_x_converted = (corner.x - map_origin_x)/(map_width_relation*map_resolution);
		   float vertex_y_converted = (map_origin_y +0.3 + input_map_height*map_resolution - corner.y)/(map_height_relation*map_resolution);
		...
		for (unsigned int j=0; j<n_places;j++)
                {
                   Place place = rooms->rooms_vector[i].places_vector[j];

                   QPoint place_point;

		   float place_x_converted = (place.x - map_origin_x)/(map_width_relation*map_resolution);
		   //float place_x_converted = (map_origin_x - place.x )/(map_width_relation*map_resolution);
