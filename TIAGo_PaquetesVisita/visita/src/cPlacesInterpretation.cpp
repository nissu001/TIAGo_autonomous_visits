...
void cPlacesInterpretation::open(ros::NodeHandle& n) {
    ...
    goalPubl = n.advertise<mensajes::Pose2DStamped>(GOAL_POSE, 1);
...
void cPlacesInterpretation::goToPlaceCallback(const std_msgs::String::ConstPtr& msg) {
    ...
    for (unsigned int i = 0; i < known_rooms.rooms_vector.size(); i++) {
        if (known_rooms.rooms_vector[i].room_name.compare(current_room) == 0) {
            ...
            //std::cout << "room places size " << known_rooms.rooms_vector[i].places_vector.size() << std::endl;
            
            for (unsigned int j = 0; j < known_rooms.rooms_vector[i].places_vector.size(); j++) {
            
            	//std::cout << "xml place " << known_rooms.rooms_vector[i].places_vector[j].place_name << "-- " << known_rooms.rooms_vector[i].places_vector[j].place_name.size() << std::endl;
                    
                //std::cout << "msg place " << msg->data << "-- " << msg->data.size() << std::endl;
